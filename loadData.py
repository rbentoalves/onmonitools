...
def read_general_info(geo: str = ""):
    ...
    site_info = all_general_info["Site Info"].set_index("Site", drop=True)

    try:
        pre_selection = pd.read_csv(pre_select_path, header=None)[0].to_list()
    except pd.errors.EmptyDataError:
        pre_selection = False

    return all_general_info, budget_prod, budget_irr, budget_pr, component_data, site_info, pre_selection


def get_daily_alarm_report(geo, analysis_start_date):
    filename = geo + '_Daily_AlarmReport_' + str(analysis_start_date) + '.xlsx'
    alarm_report = glob(os.path.join(os.getcwd(), 'Alarms Reports', geo, filename))[0]

    try:
        alarms = pd.read_parquet(alarm_report.replace('xlsx', 'parquet'), engine='pyarrow')
    except FileNotFoundError:
        alarms = pd.read_excel(alarm_report, engine="openpyxl")
        alarms.to_parquet(alarm_report.replace('xlsx', 'parquet'))

    alarms['InSolar Check'] = ''
    alarms['Comments'] = ''
...
def get_backlog_data(backlog_source, geo, analysis_start_date):
    if backlog_source == 'Event Tracker':
        filename = 'Event Tracker ' + geo + '.xlsx'
        backlog_file = glob(os.path.join(os.getcwd(), 'Event Tracker', geo, filename))[0]
        all_incidents = pd.read_excel(backlog_file, engine="openpyxl", sheet_name=["Active Events",
                                                                                   "Active tracker incidents"])

        incidents_bl = all_incidents['Active Events']
        tracker_incidents_bl = all_incidents['Active tracker incidents']

    else:
        filename = 'Reporting_' + geo + '_Sites_' + str(analysis_start_date).replace("-", "")
        backlog_files = glob(os.path.join(os.getcwd(), 'Alarms Reports', geo, filename))

        for file in backlog_files:
            all_incidents = pd.read_excel(file, engine="openpyxl", sheet_name=['Active incidents',
                                                                               'Active tracker incidents'])
            incidents_user = all_incidents['Active incidents']
            tracker_incidents_user = all_incidents['Active tracker incidents']

            try:
                incidents_bl = pd.concat([incidents_bl, incidents_user])
                tracker_incidents_bl = pd.concat([tracker_incidents_bl, tracker_incidents_user])
            except NameError:
                incidents_bl = incidents_user
                tracker_incidents_bl = tracker_incidents_user

    incidents_bl['InSolar Check'] = "x"
    tracker_incidents_bl['InSolar Check'] = "x"

    return incidents_bl, tracker_incidents_bl


def get_irradiance_day(geo, analysis_start_date):
    filename_xlsx = 'Irradiance_' + geo + '_Curated&Average-' + str(analysis_start_date).replace('-', '') + '.xlsx'
    irr_file = glob(os.path.join(os.getcwd(), 'Irradiance', geo, filename_xlsx))[0]
    irradiance_df = pd.read_excel(irr_file, engine="openpyxl")
        ...
        if "Irradiance" not in column:
            ...
            if not np_data.empty:
                ...
                try:
                    component_capacity = \
                        component_data.loc[
                            (component_data["Site"] == site) & (component_data["Component"] == component)][
...
def get_setpoint_data(site, months, SITE_INFO):
    ...
    df_setpoint["Setpoint value"] = [round(float(value.replace(',', '.')) * 1000) for value in
                                     df_setpoint["Setpoint value"]]
